...
class PlantMonitor {
    constructor() {
        ...
        // Sensor data structure - pH and temperature will be mock data at optimal levels
        this.sensorData = {
            light: { current: 0, min: 0, max: 1000, optimal: [300, 700] },
            moisture: { current: 0, min: 0, max: 100, optimal: [30, 70] },
            ph: { current: 6.5, min: 0, max: 14, optimal: [6.0, 7.5] }, // Mock data at optimal level
            temperature: { current: 22, min: 0, max: 50, optimal: [18, 25] } // Mock data at optimal level
        };
    ...
    // Fetch latest sensor data from Supabase (only light and moisture)
    async fetchSensorData() {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                // Update only light and moisture from database
                this.sensorData.light.current = latestData.light_level || 0;
                this.sensorData.moisture.current = latestData.soil_moisture || 0;
                
                // pH and temperature remain at optimal mock values
                // this.sensorData.ph.current = 6.5 (already set in constructor)
                // this.sensorData.temperature.current = 22 (already set in constructor)
    ...
    updateGaugeColors() {
        ...
        // pH gauge colors (always optimal since it's mock data)
        gauges.ph.style.background = 'linear-gradient(90deg, #4CAF50, #45a049)';
        
        // Temperature gauge colors (always optimal since it's mock data)
    ...
    calculatePlantHealth() {
        ...
        // pH and temperature are always optimal (mock data)
        optimalConditions += 2;
    ...
    // Get temperature recommendation (always optimal since it's mock data)
    ...
    showSensorDetails(card) {
        ...
        // Add mock data indicator to title if applicable
        if (isMockData) {
            modalTitle.textContent += ' (Mock Data)';
        }
    ...
    getPHParameterInfo() {
        return `
            <div class="mock-data-notice">
                ...
                <p>pH readings are currently using mock data at optimal levels (6.5 pH).</p>
            </div>
            ...
            <p><em>Note: When real pH sensors are connected, this will show actual readings and recommendations.</em></p>
        `;
    }
    ...
    getPHRecommendation() {
        return "pH is in the optimal range for most plants. This reading is currently using mock data at optimal levels.";
    }
    ...
    getTemperatureStatusInfo() {
        ...
        return `
            <div class="status-indicator good">
                <span>Perfect Temperature</span>
            </div>
            <p>Current temperature: <strong>${Math.round(temp)}°C</strong> (Mock Data)</p>
            <p>Optimal range: <strong>18-25°C</strong></p>
            <p><em>Note: This reading is currently using mock data at optimal levels.</em></p>
        `;
    }
    ...
    getDetailedTemperatureInfo() {
        ...
        return `
            <div class="mock-data-notice">
                <h4>Mock Data Active</h4>
                <p>Temperature readings are currently using mock data at optimal levels (${Math.round(temp)}°C).</p>
            </div>
            <p>Perfect temperature for healthy growth! Here's how to maintain optimal temperatures:</p>
            <ul>
